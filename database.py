...
async def get_ssh_account_list(user_id: int) -> str:
    ...
    try:
        ...
        p = subprocess.run(
            ['sudo', '/opt/julak-bot/list_ssh_users.sh'], # Panggil script list user
            capture_output=True,
            text=True,
...
async def get_vmess_account_list(user_id: int) -> str: # <--- FUNGSI BARU UNTUK VMESS LIST
    ...
    try:
        ...
        p = subprocess.run(
            ['sudo', '/opt/julak-bot/list_vmess_users.sh'], # Anda perlu membuat script ini
            capture_output=True,
            text=True,
            check=True,
            timeout=30
